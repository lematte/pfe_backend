...
module.exports.getExamenByIdCandidat_Formation = async (req, res, next) => {
    await Examen.findOne({
        isVisible: "true",
        Formation: req.params.id,
        Candidat: req.params.Candidat
    })
...
module.exports.uploadDescription = async (req, res, next) => {
    const id = req.params.id;
    const data = {
        Description: req.file.path,
    };
    const examen = Examen.findByIdAndUpdate({ _id: id }, data, { new: true })
        .then((data) => {
            res.json(data);
        })
        .catch((err) => {
